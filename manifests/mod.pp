define apache::mod (
  ...
  if $package {
    ...
    if ($::apache::apache_version == '2.4' and $::operatingsystem =~ /^[Aa]mazon$/ and $::operatingsystemrelease == '1') {
  ...
  if $_package and !defined(Package[$_package]) {
    ...
    $package_before = $::osfamily ? {
      'freebsd' => [File[$_loadfile_name], File["${::apache::conf_dir}/${::apache::params::conf_file}"]],
      default   => [File[$_loadfile_name], File[$::apache::confd_dir],],
    }
  ...
  file { $_loadfile_name:
    ...
    require => [Package['httpd'], Exec["mkdir ${mod_dir}"],],
    before  => File[$mod_dir],
    notify  => Class['apache::service'],
  }
  ...
  if $::osfamily == 'Debian' {
    ...
    file { "${_loadfile_name} symlink":
      ...
      group   => $::apache::params::root_group,
      mode    => $::apache::file_mode,
      require => [File[$_loadfile_name], Exec["mkdir ${enable_dir}"],],
    ...
    if defined(File["${mod}.conf"]) {
      file { "${mod}.conf symlink":
        ...
        group   => $::apache::params::root_group,
        mode    => $::apache::file_mode,
        require => [File["${mod}.conf"], Exec["mkdir ${enable_dir}"],],
        before  => File[$enable_dir],
        notify  => Class['apache::service'],
      }
    }
